...
import com.nicoleneza.template.v1.dtos.request.auth.PasswordResetDTO;
import com.nicoleneza.template.v1.dtos.request.auth.PasswordUpdateDTO;
...
import com.nicoleneza.template.v1.services.IAuthService;
...
public class AuthenticationController {
    ...
    public ResponseEntity<ApiResponse<UserResponseDTO>> createManager(@Valid @RequestBody CreateManagerDTO createManagerDTO) {

        logger.debug("Creating admin account for: {}", createManagerDTO.getEmail());
        UserResponseDTO createdUser = (UserResponseDTO) userService.createManagerEntity(createManagerDTO);
        return ApiResponse.success("Manager created successfully", HttpStatus.CREATED, createdUser);
    }
